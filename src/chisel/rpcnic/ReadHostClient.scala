...
class ReadHostClient() extends Module{
	val io = IO(new Bundle{
        ...
        val ser_cmd_out         = (Decoupled(new SerOutMeta))
    ...
    //send meta
    {
        val q = XQueue(new SerOutMeta, 32)  
		...
		switch(state){
            ...
            is(sDelay){
                ...
                when((delay_cnt > idle_cycle)&&(flying_cnt < outstanding_cmd)){
                    delay_cnt   := 0.U
                    state		:= sWork
                }.otherwise{
                    state		:= sDelay
    ...
    //send data
    {
        ...
        Connection.one2many(host_data_fifo.io.out)(io.ser_host_data_out, q.io.in)
        io.ser_host_data_out.bits               := host_data_fifo.io.out.bits
        q.io.in.bits                            := host_data_fifo.io.out.bits

        val arbiter							= SerialArbiter(AXIS(512), 2)

		arbiter.io.in(0)						<> io.s_mem_read_data
		...
		switch(state){
			...
			is(sWork){
				when(host_data_fifo.io.out.fire){
                    when((rpc_cnt === (total_send_num - 1.U))&(host_data_fifo.io.out.bits.last === 1.U)){
                        io.ser_host_data_out.bits.keep   := ((1.U << ser_reg.host_length(5,0)) - 1.U)
                        rpc_cnt     := rpc_cnt + 1.U
                        state		:= sEnd
                    }.elsewhen(host_data_fifo.io.out.bits.last === 1.U){
                        io.ser_host_data_out.bits.keep   := ((1.U << ser_reg.host_length(5,0)) - 1.U)
                        rpc_cnt     := rpc_cnt + 1.U
                        state		:= sWork
                    }.otherwise{
