...
class FpgaCloudMulTop extends MultiIOModule {
	...
	for(i<- 0 until 2){
		...
		// hbmDriver.io.axi_hbm(i*2) <>  withClockAndReset(hbmClk,!hbmRstn){AXIRegSlice(1)(eng_hbm(i).io.hbmAxi)}
		// hbmDriver.io.axi_hbm(i*2+1) <>  withClockAndReset(hbmClk,!hbmRstn){AXIRegSlice(1)(ser_hbm(i).io.hbmAxi)}
