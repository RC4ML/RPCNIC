...
class Deserializer128() extends Module{
    ...
    val data_fifo = XQueue(AXIS(512), entries=4096)

    io.data_in          <> data_fifo.io.in

    val data_width_convert = Module(new DataWidthConvernt)
    data_width_convert.io.data_in       <> data_fifo.io.out
	...
	val field_stack = Reg(Vec(14, new ClassMetaRsp()))
    val msg_remain_length = RegInit(VecInit(Seq.fill(14)(0.U(32.W))))
    ...
    val varint_out_len      = RegInit(UInt(5.W),0.U)
    ...
    when((state === sReadDesc) & (io.busy === 0.U)){
        io.idle_out             := 1.U
    }.otherwise{
        io.idle_out             := 0.U
    }
    ...
    switch(state){
        ...
        is(sVarintOut){
            when(io.writer_req.ready){
                ...
                when(total_remain_length === varint_out_len){
                    ...
                    string_length               := string_length - varint_out_len
                    state                       := sVarint
                }.otherwise{
                    is_repeated                 := false.B
                    state                       := sVarTag
        ...
        is(s64bit){
            when(io.writer_req.ready){
                ...
                when(total_remain_length === 8.U){
                    ...
                    string_length               := string_length - 8.U
                    state                       := s64bit
                }.otherwise{
                    is_repeated                 := false.B
                    state                       := sVarTag
                ...
                when(remain_bytes >= 8.U){
                    ...
                    when(data_in_fifo.io.out.fire()){
                        ...
                        remain_bytes                := 16.U + remain_bytes - 8.U
                        for(i<- 0 until 16){
                            data_tmp(i) := (data_in_fifo.io.out.bits.data >> ((8.U - remain_bytes)*8.U))(i*8+7,i*8)
                        }                      
                    }.otherwise{
        ...
        is(s32bit){
            when(io.writer_req.ready){
                ...
                when(total_remain_length === 4.U){
                    ...
                    string_length               := string_length - 4.U
                    state                       := s32bit
                }.otherwise{
                    is_repeated                 := false.B
                    state                       := sVarTag
                ...
                when(remain_bytes >= 4.U){
                    ...
                    when(data_in_fifo.io.out.fire()){
                        ...
                        remain_bytes                := 16.U + remain_bytes - 4.U
                        for(i<- 0 until 16){
                            data_tmp(i) := (data_in_fifo.io.out.bits.data >> ((4.U-remain_bytes)*8.U))(i*8+7,i*8)
                        }                     
                    }.otherwise{
                        io.writer_req.valid         := 0.U
                        c_msg_remain_length         := c_msg_remain_length
        ...
        is(sLengthDelimJudge){
            ...
            when(c_sub_metadata.is_repeated === true.B){
                when((c_sub_metadata.field_type === PROTO_TYPES.TYPE_STRING) ||(c_sub_metadata.field_type === PROTO_TYPES.TYPE_BYTES)) {
                    ...
                    c_msg_remain_length             := c_msg_remain_length
                    state                           := PROTO_TYPES.detailedTypeToWireType(c_sub_metadata.field_type)
        ...
        is(sLengthDelimRepeat){
            field_length_short                      := field_length(3,0)
            state                                   := sStringMoveDataFirst
                ...
                when(remain_bytes === 0.U){
                    ...
                    when(data_in_fifo.io.out.fire()){
                        ...
                        when(string_length>16.U){
                            ...
                            // when(((c_msg_remain_length === 0.U)&&(!is_repeated)) || ((c_msg_remain_length === 0.U)& (re_str_length===0.U))){
                            //     stack_num                   := stack_num - 1.U
                            // } 
                        }
                    }.otherwise{
                    ...
                    when(string_length>16.U){
                        ...
                        for(i<- 0 until 16){
                            data_tmp(i)   := data_tmp(string_length(4,0)+i.U)
                        }                                            
                        io.writer_req.bits.length   := string_length
