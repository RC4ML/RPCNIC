...
class DeserializerSimple() extends Module{
		...
		switch(state){
			is(sIdle){
				when(meta_fifo.io.out.fire){
                    ...
                    device_length := ((meta_fifo.io.out.bits.length + 63.U) >> 6.U)<<6.U
                    delay_time  := 30.U
					state		:= sDelay
				}
			}
			...
			is(sSendMeta){
				when(io.meta_out.ready){
                    ...
                    io.meta_out.bits.host_length        := 64.U
			...
			is(sSendDevice){
				when(data_fifo.io.out.fire){
                    io.device_data_out.valid           := 1.U
                    io.device_data_out.bits            := data_fifo.io.out.bits.data
