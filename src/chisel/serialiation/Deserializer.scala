...
class DeserMeta extends Bundle {
    val class_id = UInt(10.W)
    val length = UInt(32.W)
    val result_base_addr = UInt(32.W)
}
...
class Deserializer() extends Module{
    ...
    val data_width_convert = Module(new DataWidthConvernt)
    data_width_convert.io.data_in       <> io.data_in
    data_width_convert.io.meta_in       <> io.meta_in

    // val meta_in_fifo = XQueue(new DeserMeta(), entries=16)
    val data_in_fifo = XQueue(AXIS(128), entries=1024)
    ...
    // data_width_convert.io.meta_out       <> meta_in_fifo.io.in
    data_width_convert.io.data_out       <> data_in_fifo.io.in
	...
	val field_stack = Reg(Vec(8, new ClassMetaRsp()))
    val msg_remain_length = RegInit(VecInit(Seq.fill(8)(0.U(20.W))))
    val current_addr = RegInit(VecInit(Seq.fill(8)(0.U(32.W))))

    val current_base_addr = RegInit(UInt(32.W),0.U)
    val stack_num   = RegInit(UInt(3.W),0.U)

    val last = RegInit(UInt(1.W),0.U)


    
    val field_length = RegInit(UInt(16.W),0.U)
    ...
    val data_tmp            = RegInit(VecInit(Seq.fill(16)(0.U(8.W))))
    val data_pre            = RegInit(VecInit(Seq.fill(16)(0.U(8.W))))
    val data_wire           = WireInit(VecInit(Seq.fill(16)(0.U(8.W))))
    val write_tmp           = WireInit(VecInit(Seq.fill(16)(0.U(8.W))))
    val has_pre             = RegInit(false.B)
    val is_host             = RegInit(false.B)
    val data_result         = WireInit(VecInit(Seq.fill(16)(0.U(8.W))))
    val field_num           = RegInit(UInt(6.W),0.U)
    val tag                 = RegInit(UInt(3.W),0.U)
    val total_remain_length = RegInit(UInt(32.W),0.U)
    val remain_bytes        = RegInit(UInt(5.W),0.U)
    val varint_out_len      = RegInit(UInt(4.W),0.U)
    ...
    data_width_convert.io.meta_out.ready               := (state === sReadDesc) & io.class_meta_req.ready
    ...
    switch(state){
        is(sReadDesc){
            when(data_width_convert.io.meta_out.fire()){
                meta_reg                    := data_width_convert.io.meta_out.bits
                total_remain_length         := data_width_convert.io.meta_out.bits.length
                current_base_addr           := data_width_convert.io.meta_out.bits.result_base_addr
                io.class_meta_req.valid          := 1.U
                io.class_meta_req.bits.class_id  := data_width_convert.io.meta_out.bits.class_id
                state                       := sWriteMeta
            }
        }
        is(sWriteMeta){
            when(class_meta_rsp_fifo.io.out.fire()){
                ...
                current_addr(stack_num)     := current_base_addr
        ...
        is(sVarTag){
            when((c_msg_remain_length === 0.U)&&(stack_num =/= 0.U)){
                ...
                }.elsewhen(data_in_fifo.io.out.fire()){
        ...
        is(sReadTag){
            ...
            when(remain_bytes >= varint_out_len){
                ...
                for(i<- 0 until 16){
				    data_tmp(i) := data_tmp((i.U + varint_out_len)%16.U)
			    }     
                state                       := sJudgeField         
            }.elsewhen(has_pre){             
                state                       := sJudgeField
                remain_bytes                := 16.U + remain_bytes - varint_out_len
                for(i<- 0 until 16){
        ...
        is(sVarintOut){
            when(io.writer_req.ready){
                ...
                io.writer_req.bits.addr         := current_addr(stack_num)
                ...
                when(PROTO_TYPES.detailedTypeToCppSizeLog2(c_sub_metadata.field_type) === 2.U){
                    ...
                    current_addr(stack_num)     := current_addr(stack_num) + 4.U                      
                }.otherwise{
                    io.writer_req.bits.length   := 8.U
                    current_addr(stack_num)     := current_addr(stack_num) + 8.U
                }
        ...
        is(s64bit){
            when(io.writer_req.ready){
                ...
                when(remain_bytes >= 8.U){
                    ...
                    remain_bytes                := 16.U + remain_bytes - 8.U
                    for(i<- 0 until 16){
                    ...
                    when(data_in_fifo.io.out.fire()){
                        ...
                        current_addr(stack_num)     := current_addr(stack_num)
                        c_msg_remain_length         := c_msg_remain_length
                        total_remain_length         := total_remain_length
                        stack_num                   := stack_num
                        state                       := s64bit
        ...
        is(s32bit){
            when(io.writer_req.ready){
                ...
                when(remain_bytes >= 4.U){
                    ...
                    remain_bytes                := 16.U + remain_bytes - 4.U
                    for(i<- 0 until 16){
        ...
        is(sStringMoveDataFirst){
            when(io.writer_req.ready){
                ...
                io.writer_req.bits.addr         := current_addr(stack_num)
                io.writer_req.bits.is_host      := c_sub_metadata.is_host
                current_addr(stack_num)         := current_addr(stack_num) + field_length
                ...
                when(remain_bytes >= field_length){
                    ...
                    when(has_pre){
                        when(field_length <= 16.U){
                            ...
                            io.writer_req.bits.length   := 16.U
                            state                       := sStringMoveData 
                            string_length               := string_length - 16.U        
                        }
                    }.otherwise{
                        io.writer_req.valid             := 0.U
                        state                       := sStringMoveData
                ...
                when(remain_bytes === 0.U){
                    ...
                    when(data_in_fifo.io.out.fire()){
                        ...
                        when(string_length>16.U){
                            ...
                            remain_bytes                := 16.U - string_length
