...
int main(int argc, char *argv[]) {
    ...
    eng_cmd.src_length = (host_serialized_len + 63) / 64 * 64;
    ...
    while (fpga_ctl->readReg(512 + 420) != ((host_serialized_len + 63) / 64)) {
    };

    auto end = chrono::high_resolution_clock::now();
    std::chrono::duration<double> diff_2 = middle - start;
    std::chrono::duration<double> millis_2 = diff_2 * 1000 * 1000;
    cout << "1. Hyperprotobench.bench0.sw.Serialize() timeing: " << millis_2.count() << " us\n";
    std::chrono::duration<double> diff_1 = end - start;
    std::chrono::duration<double> millis_1 = diff_1 * 1000 * 1000;
    cout << "1. Hyperprotobench.bench0.sw + hw Total() timeing: " << millis_1.count() << " us\n";
