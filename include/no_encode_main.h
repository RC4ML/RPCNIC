...
uint8_t *SendMetadataToFPGA_hw(M_base *message, size_t *p_c2h, size_t *messageSizeVec, FPGA_Metadata *FPGA_metadataVec, size_t &p_c2h_buffer_size, size_t &min_class_id, size_t &max_class_id) {
    uint8_t *uint8_p_c2h = reinterpret_cast<uint8_t *>(p_c2h);
    void *obj_addr = message;
    void *obj_addr_base = obj_addr;
    obj_addr += 8;  //skip the pointer to virtual function table
    ...
    for (int i = current_metadata.field_num - 1; i >= 0; --i) {
        ...
        case sw_repeated_int32_type:
        {
            ...
            FPGA_metadataVec[current_class_id].field[current_field_id].field_type = 3 | (FPGA_TYPE_INT32 << 2);
            FPGA_metadataVec[current_class_id].field[current_field_id].reserve = 0;
            FPGA_metadataVec[current_class_id].field[current_field_id].sub_class_id = current_field.size();
        }
        break;
        case sw_int64_type:
        {
        ...
        case sw_string_type:
        {
            ...
            FPGA_metadataVec[current_class_id].field[current_field_id].reserve = 0;
            FPGA_metadataVec[current_class_id].field[current_field_id].sub_class_id = (aligned_size / 64);
        }
        break;
        case sw_repeated_string_type:
        {
        ...
        case sw_ptr_type:
        {
            ...
            FPGA_metadataVec[current_class_id].field[current_field_id].reserve = 0;
            FPGA_metadataVec[current_class_id].field[current_field_id].sub_class_id = new_obj_metadata.class_id;
            uint8_p_c2h = SendMetadataToFPGA_hw(reinterpret_cast<M_base *>(current_field), reinterpret_cast<size_t *>(uint8_p_c2h), messageSizeVec, FPGA_metadataVec, p_c2h_buffer_size, min_class_id, max_class_id);
        }
        break;
        default:
            break;
