...
inline uint8_t *WriteRepeatedBoolToArray(int field_number, vector<bool> &value, uint8_t *target) {
    target = WriteTagToArray(field_number, PROTOBUF_LENGTH_DELIMITED_TYPE, target);
    uint32_t length = 0;
    for (int j = 0; j < value.size(); ++j) {
        length += 1;
...
inline uint8_t *WriteSkipRepeatedStringToArray(int field_number, vector<string> &value, uint8_t *target, string &skip_field, bool skip_flag) {
    target = WriteTagToArray(field_number, PROTOBUF_LENGTH_DELIMITED_TYPE, target);
    uint32_t length = 0;
    for (int j = 0; j < value.size(); ++j) {
        if ((value[j].size() > 1024) && skip_flag) {
    ...
    target = WriteVarint32ToArray(length, target);
    for (int i = 0; i < value.size(); i++) {
        if ((value[i].size() > 1024) && skip_flag) {
