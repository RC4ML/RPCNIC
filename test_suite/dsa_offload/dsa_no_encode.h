...
//        << decode <<
#define USE_DSA
constexpr size_t use_dsa_size = 256;
std::vector<dml_job_t *> jobs_list;
size_t now_job_idx = 0;
...
#ifdef USE_DSA
    if (s_size > use_dsa_size) {
        dml_job_t *now_job = jobs_list[now_job_idx];
        now_job->source_first_ptr = reinterpret_cast<uint8_t *>(const_cast<char *>(s.c_str()));
        now_job->destination_first_ptr = target;
        now_job->source_length = s_size;
        now_job->destination_length = s_size;
